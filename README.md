# golang ![](https://travis-ci.org/droboports/golang.svg?branch=master)
...
Check the [releases](https://github.com/droboports/golang/releases) page. If there are no releases available, then you have to compile.
...
```
git clone https://github.com/droboports/golang.git
cd golang
./build.sh
ls -la *.tgz
```
...
```
./build.sh clean
XGOLANG="${HOME}/xtools/golang/5n"
BUILD_DEST="${XGOLANG}" ./build.sh
cp -vfaR ./target/install/bin/* "${XGOLANG}/bin/"
cp -vfaR ./target/install/include/* "${XGOLANG}/include/"
```
